...
function setup_vendor() {
    ...
    export PR_ROOT="$3"
    if [ ! -d "$PR_ROOT" ]; then
        echo "\$PR_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PR_ROOT/$OUTDIR" ]; then
        mkdir -p "$PR_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$PR_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$PR_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PR_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PR_ROOT"/vendor/pr/build/tools/smali/baksmali.jar
        export SMALIJAR="$PR_ROOT"/vendor/pr/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PR_ROOT"/vendor/pr/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PR_ROOT"/vendor/pr/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if grep "classes.dex" "$PR_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$PR_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$PR_ROOT"/vendor/pr/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$PR_ROOT"/"$OUTDIR"/radio
